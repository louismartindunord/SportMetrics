...
def import_all_data(user):
    try:
        ...
        columns = ["date","sport","exercice","duree","seance","poid","nombre_repetition"]
...
def plot_sport_frequence(df:pd.DataFrame, date_options:str):
    ...
    df_timeseries['did_sport_this_day'] = df_timeseries['date'].apply(lambda x: 1 if x in df['date'].values else 0)
